...
function getEndpointCandidates(baseUrl: string, classId: string, predictionId: string): string[] {
  const normalizedBase = baseUrl.replace(/\/+$/, '');
  const endpoint = `/teacher/classes/${classId}/predictions/${predictionId}`;
...
export async function GET(
  ...
  { params }: { params: Promise<{ classId: string; predictionId: string }> }
) {
  try {
    ...
    const { classId, predictionId } = await params;
    ...
    const endpoints = getEndpointCandidates(API_BASE_URL, classId, predictionId);
    let lastStatus = 502;
    let lastMessage = 'Failed to fetch prediction details';
    ...
    console.error('Error fetching teacher prediction details:', error);
    return NextResponse.json(
