...
function getEndpointCandidates(baseUrl: string, classId: string): string[] {
  ...
  const endpoint = `/teacher/classes/${classId}/students/prediction-status-aggregated`;
...
export async function GET(
  ...
  try {
    ...
    let lastStatus = 502;
    let lastMessage = 'Failed to fetch aggregated class students';
    ...
    console.error('Error fetching aggregated class students prediction status:', error);
    return NextResponse.json(
