...
function getEndpointCandidates(baseUrl: string): string[] {
  ...
  const endpoint = '/teacher/predictions/metrics';
...
export async function GET() {
  try {
    ...
    let lastStatus = 502;
    let lastMessage = 'Failed to fetch prediction metrics';
    ...
    console.error('Error fetching teacher prediction metrics:', error);
    return NextResponse.json(
