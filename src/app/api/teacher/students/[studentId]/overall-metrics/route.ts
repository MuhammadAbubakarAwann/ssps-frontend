...
function getEndpointCandidates(baseUrl: string, studentId: string): string[] {
  ...
  const endpoint = `/teacher/students/${studentId}/overall-metrics`;
...
export async function GET(
  ...
  try {
    ...
    let lastStatus = 502;
    let lastMessage = 'Failed to fetch student overall metrics';
    ...
    console.error('Error fetching student overall metrics:', error);
    return NextResponse.json(
