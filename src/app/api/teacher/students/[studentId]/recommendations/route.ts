...
function getEndpointCandidates(baseUrl: string, studentId: string): string[] {
  ...
  const endpoint = `/teacher/students/${studentId}/recommendations`;
...
export async function GET(
  ...
  try {
    ...
    let lastStatus = 502;
    let lastMessage = 'Failed to fetch recommendations';
    ...
    console.error('Error fetching teacher student recommendations:', error);
    return NextResponse.json(
