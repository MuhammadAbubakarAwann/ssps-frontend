...
function getEndpointCandidates(baseUrl: string, studentId: string): string[] {
  ...
  const endpoint = `/teacher/students/${studentId}/latest-predictions`;
...
export async function GET(
  ...
  try {
    ...
    let lastStatus = 502;
    let lastMessage = 'Failed to fetch latest predictions';
    ...
    console.error('Error fetching teacher student latest predictions:', error);
    return NextResponse.json(
