...
interface RejectRestaurantDialogProps {
  restaurant: Restaurant;
  onReject: (restaurantId: string, reason: string) => void;
  onCancel: () => void;
}

export function RejectRestaurantDialog({ restaurant, onReject, onCancel }: RejectRestaurantDialogProps) {
  const [rejectionReason, setRejectionReason] = useState('');
  const [isOpen, setIsOpen] = useState(true);

  const handleSubmit = () => {
    if (rejectionReason.trim()) {
      onReject(restaurant.id, rejectionReason.trim());
      setRejectionReason('');
      setIsOpen(false);
    }
  };
        ...
        >
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className='text-muted-foreground mt-2 text-sm'>
              Are you sure you want to reject {restaurant.restaurantName}? This action cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          ...
          <div className='mt-4'>
            ...
            <textarea
              ...
              placeholder='Please provide a detailed reason for rejecting this restaurant...'
