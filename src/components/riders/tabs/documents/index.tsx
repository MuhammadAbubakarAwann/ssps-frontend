...
import { fetchRiderDocuments, type RiderDocumentsResponse } from '@/lib/server-actions/rider-actions';

// Document interface for UI
interface DocumentForUI {
  id: string;
  category: string;
  filename: string;
  url: string;
...
const DOCUMENT_NAMES: Record<string, string> = {
  ...
  workEligibilityCanada: 'Work Eligibility (Canada)',
...
export default function RiderDocumentsTab({ riderId }: RiderDocumentsTabProps) {
  const [selectedDocument, setSelectedDocument] = useState<DocumentForUI | null>(null);
  const [imageViewerOpen, setImageViewerOpen] = useState(false);
  const [documentsData, setDocumentsData] = useState<RiderDocumentsResponse | null>(null);
  ...
  useEffect(() => {
    const loadDocuments = async () => {
      try {
        setLoading(true);
        setError(null);
        const data = await fetchRiderDocuments(riderId);
  ...
  const documents: DocumentForUI[] = documentsData
    ? Object.entries(documentsData.data.documents).map(([key, url], index) => ({
        id: `${key}_${index}`,
        category: DOCUMENT_NAMES[key] || key.replace(/([A-Z])/g, ' $1').replace(/^./, str => str.toUpperCase()),
        filename: getFilenameFromUrl(url),
        url: url,
        uploaded: true, // All documents in the response are uploaded
        required: true  // All rider documents are required
      }))
    : [];
